...
static void X_CompareAsserts(CuTest *tc, const char *file, int line, const char *message,
...
void TestCuSuiteSummary(CuTest *tc) {
    ...
    CuSuiteInit(&ts);
    char *passes = "TestPasses";
    CuTestInit(&tc1, passes, TestPasses);
    char *fails = "TestFails";
    CuTestInit(&tc2, fails,  zTestFails);
    ...
    char result[32];
    char buffer[32];
    char *offset = summary.buffer;

    snprintf(result, sizeof result, "%s%s", "SUCCESS | ", passes);
    offset = strchr(offset, '|');
    snprintf(buffer, strlen(result) + 1, "%s", offset + 2);
    CuAssertStrEquals(tc, result, buffer);
    offset += strlen(result);

    snprintf(result, sizeof result, "%s%s", "FAILURE | ", fails);
    offset = strchr(offset, '|');
    snprintf(buffer, strlen(result) + 1, "%s", offset + 2);
    CuAssertStrEquals(tc, result, buffer);
}
